...
# https://raw.githubusercontent.com/jmahlman/Mac-Admin-Scripts/master/Adobe-RUMWithProgress-jamfhelper.sh
#
...
# Changelog
#
# 23/10/12 - two echo statements to clean up the displayed output, bash duration for elapsed time.
...
SECONDS=0
icons="/Applications/Utilities/Adobe Creative Cloud/ACC/Creative Cloud.app/Contents/Resources/"
...
installUpdates ()
{
    ...
    "$jamfHelper" -windowType hud -title "Adobe Updater" -description "Downloading and installing updates, this may take 5-10 minutes..." \
    -icon "$icons/CreativeCloudApp.icns" -lockHUD > /dev/null 2>&1 &
    # do all of your work here
    $rum --action=install > /dev/null 2>&1
    # Kill jamfhelper
...
# temporarily disabling the RUM check so we force the latest version
...
secho=$(echo "$secho" | tr -d '()*' | sed -e 's/\// /g' -e 's/\(.*\)\..*/\1/' ) # clean up the displayed update name

if [ "$(grep -i "updates are available:" "$rumlog")" ] ; then
  userChoice=$("$jamfHelper" -windowType hud -lockHUD -title "Adobe Updater" \
  -icon "$icons/CreativeCloudApp.icns" -description "Do you want to install these updates?
...
$secho" -button1 "Yes" -button2 "No")
    if [[ "$userChoice" == "0" ]]; then
        ...
        -icon "$icons/CreativeCloudApp.icns" -button1 Ok -defaultButton 1
