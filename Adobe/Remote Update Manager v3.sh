...
# IM misc
#
# remember to update the dialog titles to remove 'UArts' after 
# installCocoaDialog is policy 156, make sure custom trigger matches
...
installUpdates ()
{
	...
	$CocoaDialog progressbar --indeterminate --float --icon-file "$icons/Sync.icns" \
		--title "Adobe Updater" --text "Downloading and Installing Updates, this may take some time..." \
...
if [ "$(grep "Following Updates are applicable" $rumlog)" ] ; then
	rv=`$CocoaDialog yesno-msgbox --float --icon-file "$icons/ToolbarInfo.icns" --no-cancel \
		--title "Adobe Updater" --text "Do you want to install the following updates?" --informative-text "$secho"`
	if [ "$rv" == "1" ]; then
		installUpdates
	elif [ "$rv" == "2" ]; then
		exit 0
	...
	$CocoaDialog ok-msgbox --float --no-cancel --icon-file "$icons/ToolbarInfo.icns" \
		--title "Adobe Updater" --text "There are no Adobe Updates available."
	if [ "$rv" == "1" ]; then
		exit 0
	fi
fi
